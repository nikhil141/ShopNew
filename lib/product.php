...
if(isset($_POST['save']))
{
	 ...
	 $utype = $_POST['unit_type'];
	 $bcode = $_POST['bar_code'] ;
	 
    //$date = date('Y-m-d');

   
   
    $sql = mysqli_query($con,"INSERT INTO tbl_product (`product_name`, `brand`, `purchase_price`,`sale_price`, `unit_type`, `bar_code`)VALUES('$pname', '$brand', '$pprice' ,'$sprice', '$utype', '$bcode')");  
...
$sql=mysqli_query($con,"UPDATE `tbl_product` SET `product_name` = '$pname' , `brand` = '$brand', `purchase_price` = '$pprice', `sale_price`='$sprice',`unit_type` = '$utype',`bar_code` = '$bcode' WHERE `product_id` = '$id' ");
